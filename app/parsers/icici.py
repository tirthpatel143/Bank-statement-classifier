...
class ICICIParser(BankParser):
    ...
    def can_parse(self, text: str) -> bool:
        ...
        return "ICICI BANK" in text_upper or "ICICIBANK" in text_upper or "ICIC0" in text_upper

    def extract_account_details(self, pages_data: List[Dict[str, Any]]) -> AccountDetails:
        return extract_dynamic_account_details(pages_data, default_bank="ICICI Bank Limited")
                ...
                if match:
                    ...
                    if parsed_date:
                        ...
                        while idx < len(lines) and not re.match(date_pattern, lines[idx].strip()) and lines[idx].strip():
                            if "TRANSACTION REMARKS" in lines[idx].upper() or "PAGE" in lines[idx].upper():
                                break
                            row_lines.append(lines[idx].strip())
                            idx += 1
                        
                        full_row = " ".join(row_lines)
                        tx = self._parse_icici_line(full_row, parsed_date, page_num)
                        if tx:
    ...
    def _parse_icici_line(self, line: str, parsed_date: str, page_num: int) -> Optional[Transaction]:
        ...
        if len(amounts) >= 3:
            ...
            if amt_val:
                amount = amt_val
                if "CR" in line.upper() or "INF/" in line.upper() or "BY " in line.upper():
                    credit = amt_val
                    tx_type = "credit"
                else:
                    debit = amt_val
