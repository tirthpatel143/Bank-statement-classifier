...
class SBIParser(BankParser):
    ...
    def can_parse(self, text: str) -> bool:
        ...
        return "STATE BANK OF INDIA" in text_upper or "SBI" in text_upper or "SBIN" in text_upper

    def extract_account_details(self, pages_data: List[Dict[str, Any]]) -> AccountDetails:
        return extract_dynamic_account_details(pages_data, default_bank="State Bank of India")
                ...
                if match:
                    ...
                    if parsed_date:
                        ...
                        while idx < len(lines) and not re.match(date_pattern, lines[idx].strip()) and lines[idx].strip():
                            if "TXN DATE" in lines[idx].upper() or "PAGE" in lines[idx].upper():
                                break
                            row_lines.append(lines[idx].strip())
                            idx += 1
                            
                        full_row = " ".join(row_lines)
                        tx = self._parse_sbi_line(full_row, parsed_date, page_num)
                        if tx:
    ...
    def _parse_sbi_line(self, line: str, parsed_date: str, page_num: int) -> Optional[Transaction]:
        ...
        if len(amounts) >= 3:
            d_val, _ = clean_amount(amounts[-3])
            c_val, _ = clean_amount(amounts[-2])
            ...
            if amt_val:
                amount = amt_val
                if "CR" in line.upper() or "BY " in line.upper() or "TRANSFER FROM" in line.upper() or "CREDIT" in line.upper():
                    credit = amt_val
                    tx_type = "credit"
                else:
                    debit = amt_val
        ...
        desc = line
        desc = re.sub(r'^\s*(?:\d+[\s.-]+)?(?:\d{1,2}\s+[A-Za-z]{3}\s+\d{4}|\d{1,2}[/-]\d{1,2}[/-]\d{2,4}|\d{1,2}[/-][A-Za-z]{3}[/-]\d{2,4})', '', desc).strip()
        for am in amounts:
            desc = desc.replace(am, '')
        desc = re.sub(r'\s+', ' ', desc).strip()
        if not desc:
            desc = "SBI Transaction"
