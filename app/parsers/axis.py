...
class AxisParser(BankParser):
    ...
    def can_parse(self, text: str) -> bool:
        ...
        return "AXIS BANK" in text_upper or "UTIB" in text_upper

    def extract_account_details(self, pages_data: List[Dict[str, Any]]) -> AccountDetails:
        return extract_dynamic_account_details(pages_data, default_bank="Axis Bank Limited")
            ...
            while idx < len(lines):
                ...
                if match:
                    ...
                    if parsed_date:
                        ...
                        tx = self._parse_axis_line(full_row, parsed_date, page_num)
                        if tx:
    ...
    def _parse_axis_line(self, line: str, parsed_date: str, page_num: int) -> Optional[Transaction]:
        ...
        if len(amounts) >= 3:
            ...
            if amt_val:
                amount = amt_val
                if "CR" in line.upper() or "BY " in line.upper():
                    credit = amt_val
                    tx_type = "credit"
                else:
                    debit = amt_val
